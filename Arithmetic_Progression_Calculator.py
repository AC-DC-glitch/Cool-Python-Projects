...
def ap_func():
    try:
        ...
        ask = input("Do you want the AP up to n terms? Y/N?:")
        ...
        if cd == "" and nsum == "":
            ...
            print("The common difference is ", cd, "The sum of the first n terms is ", nsum)

        if nth_term == "" and nsum == "" and ask == "Y":
            nth_term = (term_a + ((nterm - 1) * cd))
            nsum = (nterm / 2) * (2 * term_a + (nterm - 1) * cd)
            print("The nth term is ", nth_term, "The sum of the 1st n terms of the AP is ", nsum, "and your AP is:")
            x = 1
            while x <= nterm:
                print(term_a + (x - 1) * cd)
                x += 1

        if nth_term == "" and nsum == "" and ask == "N":
            nth_term = (term_a + ((nterm - 1) * cd))
            nsum = (nterm / 2) * (2 * term_a + (nterm - 1) * cd)
            print("The nth term is ", nth_term, "The sum of the 1st n terms of the AP is ", nsum)
    except:
        print("Invalid Input")
